...
const notices_help = "Generate a release-ready third-party licence notices text file from locked dependencies. Products with identical licence text are grouped so the shared text is emitted once; package-specific NOTICE content remains separate. Each package's own source archive is used first; when it ships no licence text the command falls back to the declared repository (GitHub, Codeberg, or GitLab, at an immutable tag commit) and then to canonical SPDX License List text. A transient repository failure is non-fatal: it warns and continues to the SPDX fallback."
