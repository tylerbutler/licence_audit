...
fn binary_present() -> Bool {
  case simplifile.read(from: toml_port.binary_path()) {
    Ok(_) -> True
    Error(_) -> False
  }
}
  ...
  case binary_present() {
    ...
    True -> {
      ...
      let assert Ok(output) =
        toml_port.set_string_array(input, ["tools", "licence_audit"], "allow", [
          "MIT",
        ])
...
pub fn preserves_inline_trailing_comment_test() {
  case binary_present() {
    ...
    True -> {
      ...
      should.be_true(string.contains(output, "\"Apache-2.0\""))
    }
  }
}
...
pub fn empty_input_creates_nested_table_test() {
  case binary_present() {
    ...
    True -> {
      let assert Ok(output) =
        toml_port.set_string_array("", ["tools", "licence_audit"], "allow", [
          "MIT",
        ])
