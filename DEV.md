...
just ci             # full validation (format-check + glint + check + test + strict build + docs-check)
just clean          # remove build artifacts
```

Run `just` with no arguments to see the full recipe list. GitHub Actions also
runs `just sbom-validate` after the regular validation flow.
